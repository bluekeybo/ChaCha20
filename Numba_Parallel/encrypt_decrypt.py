...
def parallel(func, chunks, count_start):
    # Because we're using multiprocessing, counter needs to be pre-computed
...
def main():
    ...
    if args.encrypt:
        ...
        chunks = [file_in[i : i + 64] for i in range(0, len(file_in), 64)]

        file_out = salt + IV + parallel(cipher.encrypt, chunks, counter)
    ...
    # Decryption
        ...
        file_out = parallel(cipher.decrypt, chunks, counter)
